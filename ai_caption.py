...
# Channel-subscribe CTA appended to the bottom of every caption
SUBSCRIBE_LINE = "🔔 اشترك دلوقتي في القناة وما تفوتش أقوى العروض!"
...
class CaptionGenerator:
    ...
    @staticmethod
    def _finalize(caption):
        """Trim to Telegram's caption limit, then append the subscribe CTA."""
        footer = "\n\n" + SUBSCRIBE_LINE
        limit = 1024
        if len(caption) + len(footer) > limit:
            caption = caption[: limit - len(footer) - 3].rstrip() + "..."
